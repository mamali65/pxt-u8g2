...
namespace OLEDV1 {
    ...
    export function initDisplay(): void {
        ...
        cmd(0x1F);  // Multiplex Ratio for 128x64 (64-1)
        ...
        cmd(0x20);  // Set Segment Re-Map
        cmd(0x02);  // Set Com Output Scan Direction
        cmd(0xA1);  // Set COM Hardware Configuration
        cmd(0xC8);  // COM Hardware Configuration
        cmd(0xDA);  // Set Contrast
        cmd(0x02);  // Contrast
        cmd(0x81);  // Set Pre-Charge Period
        cmd(0x8f);  // Set Pre-Charge Period (0x22 External, 0xF1 Internal)
        cmd(0xD9);  // Set VCOMH Deselect Level
        cmd(0xf1);  // VCOMH Deselect Level
        cmd(0xDB);  // Set all pixels OFF
        cmd(0x40);  // Set display not inverted
        cmd(0xA4);  // Set display On
        cmd(0xA6);  // Set display On
        cmd(0x2E);  // Set display On
        cmd(0xAF);
        clear();
    }
    //%weight=60
